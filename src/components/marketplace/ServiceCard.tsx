...
interface ServiceCardProps {
  ...
  // If set, there's an active request for this service type (manuscript context only)
  activeRequestId?: string;
}

export default function ServiceCard({ service, manuscriptId, userDisplayName, activeRequestId }: ServiceCardProps) {
  ...
  const hasActiveRequest = Boolean(activeRequestId);
    ...
    <>
      <div className="flex h-full flex-col justify-between rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:border-blue-200 hover:shadow-md">
        ...
        <div className="mt-6 pt-4 border-t border-slate-100">
          {hasActiveRequest ? (
            // Show View Order button when there's an active request
            <Link
              href={`/dashboard/orders/${activeRequestId}`}
              className="block w-full rounded-lg bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-700 text-center shadow-sm transition-colors hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2"
            >
