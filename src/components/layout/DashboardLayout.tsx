...
function getVisibleNavItems(items: NavItem[], isAdmin: boolean) {
  return items.filter((item) => {
    ...
    // Hide Admin tools from Non-Admins // No longer needed as we removed the Admin item, but kept for future
        ...
        <nav className="mt-6 px-3">
          <ul className="space-y-1">
            {visibleNavItems.map((item) => {
              ...
              if (item.name === "Support" && (user.role === "support_agent" || user.role === "admin" || user.role === "super_admin")) {
                finalName = "Support Queue";
              }

              // Active state logic:
              // 1. Exact match
              // 2. "Dashboard" item is active if path starts with /dashboard/admin and user is admin
              const isActive = pathname === finalHref || 
                (item.name === "Dashboard" && isAdmin && pathname.startsWith("/dashboard/admin"));
