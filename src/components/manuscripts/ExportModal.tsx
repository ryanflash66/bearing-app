...
function ExportModalContent({
  ...
  const handleDownload = async (format: "pdf" | "docx") => {
    ...
    /**
     * Use a direct navigation-style download.
     *
     * Chromium can block programmatic blob downloads after an async `fetch`
     * because user-activation is lost. A direct `a[href]` click reliably
     * triggers the browser's native download handling (Content-Disposition).
     */
    try {
      ...
      console.error("Export download failed to start:", {
        format,
        manuscriptId,
        error: e instanceof Error ? e.message : String(e),
        timestamp: new Date().toISOString(),
      });
      setExportError("Export failed to start. Please try again.");
    } finally {
      // We can't observe download completion; release the UI quickly.
        ...
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
          ...
          <div className="w-full md:w-80 border-b md:border-b-0 md:border-r bg-slate-50 p-4 md:p-6 flex flex-col gap-4 md:gap-6 overflow-y-auto max-h-[40vh] md:max-h-none">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateSettings({ fontFace: e.target.value as any })
                  }
            ...
            <div className="flex items-center justify-between">
              ...
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings.isLivePreview ? "translate-x-6" : "translate-x-1"}`}
                />
              </button>
            </div>
          ...
          <div
            className="flex-1 bg-slate-100 overflow-hidden relative min-h-[30vh] md:min-h-0"
            ...
            {settings.isLivePreview ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center text-slate-400 p-4">
                <div className="text-center">
                  <svg
                    className="mx-auto h-12 w-12 md:h-16 md:w-16 text-slate-300 mb-4"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
