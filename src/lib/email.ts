...
export async function notifyAdminsNewTicket(
  ...
  if (!result.success) {
    console.error("[NOTIFY_ADMIN_NEW_TICKET_FAILED]", {
      ticketId,
      error: result.error,
    });
  }
...
export async function notifyUserReply(
  ...
  if (!result.success) {
    console.error("[NOTIFY_USER_REPLY_FAILED]", {
      ticketId,
      userEmail,
      error: result.error,
    });
...
export async function notifyAdminReply(
  ...
  if (!result.success) {
    console.error("[NOTIFY_ADMIN_REPLY_FAILED]", {
      ticketId,
      error: result.error,
    });
  }
...
export async function notifyServiceFulfilled(
  ...
  if (!result.success) {
    console.error("[NOTIFY_SERVICE_FULFILLED_FAILED]", {
      userEmail,
      serviceType,
      error: result.error,
    });
...
export async function notifyServiceCancelled(
  ...
  if (!result.success) {
    console.error("[NOTIFY_SERVICE_CANCELLED_FAILED]", {
      userEmail,
      serviceType,
      error: result.error,
    });
...
export async function notifyBlogPostSuspended(
  ...
  if (!result.success) {
    console.error("[NOTIFY_BLOG_POST_SUSPENDED_FAILED]", {
      userEmail,
      postTitle,
      error: result.error,
    });
...
export async function sendOrderStatusEmail(
  ...
  if (!result.success) {
    console.error("[NOTIFY_ORDER_STATUS_CHANGE_FAILED]", {
      userEmail,
      orderId,
      serviceType,
      newStatus,
