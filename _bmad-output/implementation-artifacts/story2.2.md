...
| 2025-12-29 | **Snapshot Identity Fix**: Resolved Foreign Key violation by mapping Auth ID to Public User ID. | Dev Agent |
| 2025-12-29 | **Premium UX**: Replaced browser confirm with custom `VersionRestoreModal`. | Dev Agent |
| 2025-12-29 | **Editor Sync**: Fixed Tiptap synchronization to instantly reflect restored versions. | Dev Agent |

## Maintenance & Hardening (2025-12-29)

### Technical Improvements:
- **ID Mapping Correction**: Fixed a critical bug in `createVersionSnapshot` where the system attempted to use the Supabase Auth ID in the `created_by` field, which effectively required the Public User ID from the `users` table.
- **Instant Restoration Sync**: 
    - Updated `restoreVersion` to return the updated manuscript data.
    - Modified `ManuscriptEditor` to immediately set local state (`setContent`, `setTitle`) and `resetTimestamp` upon successful restoration.
    - **Tiptap Sync**: Re-enabled and optimized the `useEffect` hook in `TiptapEditor.tsx` to listen for external content changes and update via `editor.commands.setContent`, enabling instant visual restoration without page reloads.
- **Premium Restore Flow**: 
    - Created `VersionRestoreModal.tsx` with a high-fidelity "Modern Parchment" aesthetic.
    - Integrated with `VersionHistory` to replace standard browser behavior with an immersive, safe-restore experience.
- **Error Transparency**: Refactored the snapshot logic to correctly identify and log database constraints (e.g., uniqueness, foreign keys) instead of returning empty error objects.