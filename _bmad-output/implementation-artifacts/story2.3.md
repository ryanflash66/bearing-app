...
### AC 2.3.6 (UX Pattern - Ghost Text)

- **Given:** I am actively typing in the editor
- **When:** I pause for 3 seconds without further input
- **Then:** A subtle AI suggestion appears as "ghost text" (SF Mono, muted color) after my cursor
- **And:** Pressing `Tab` accepts the suggestion (text merges, cursor advances)
- **And:** Pressing `Esc` dismisses the suggestion (ghost text fades)
...
### UX Rework Tasks (AC 2.3.6 - Ghost Text)

- [x] Create `useGhostText` hook to detect 3-second typing pause
- [x] Implement ghost text display after cursor (SF Mono, muted color)
- [x] Add `Tab` key handler to accept suggestion
- [x] Add `Esc` key handler to dismiss suggestion
- [x] Create fade animation for ghost text appearance/dismissal
- [x] Integrate with existing Llama suggestion API
- [x] Add unit tests for `useGhostText` hook
- [x] Debounce API calls to prevent excessive requests (3s pause timer)
...
- `src/lib/useGhostText.ts` - Ghost text hook with 3s pause detection
- `src/components/manuscripts/GhostTextDisplay.tsx` - Ghost text display component
- `tests/lib/useGhostText.test.ts` - Unit tests for ghost text hook

### Modified Files
- `src/components/manuscripts/ManuscriptEditor.tsx` - Added AI suggestion functionality with text selection and suggestion UI, ghost text integration
...
- `src/app/globals.css` - Added ghost text styling and animations
...
| 2025-12-29 | **UX Rework:** Implemented ghost text pattern (AC 2.3.6) | Dev Agent (Amelia) |
| 2025-12-29 | Created useGhostText hook with 3s pause detection | Dev Agent (Amelia) |
| 2025-12-29 | Added Tab/Esc keyboard shortcuts for accept/dismiss | Dev Agent (Amelia) |
| 2025-12-29 | Created GhostTextDisplay component with SF Mono styling | Dev Agent (Amelia) |